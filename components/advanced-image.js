app.component('advanced-image', {
    ...
    template: `
    <div class="advanced-image" :style="{padding: (canvas_size/2)+'px', borderRadius: (canvas_size/2)+'px'}">
        ...
        <img :style="imageStyle" src="./assets/images/firefox_logo_2017.png" @mousemove="getMousePos($event)" @mouseenter="imageFocus=true" @mouseleave="imageFocus=false">
		<p v-show="false">{{ debug }}</p>
    </div>`,
    methods: {
        draw() {
            let image = document.getElementById(`${this.name}-source`);
            let fa = this.image_size / (
            ...
            ctx.drawImage(source, this.x, this.y, this.canvas_size, this.canvas_size, 0, 0, this.canvas_size, this.canvas_size);
        },
        getMousePos(event) {
			console.log(event);
